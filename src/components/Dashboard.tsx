...
export function Dashboard({
  ...
  return (
    <div style={{ maxWidth: '1360px', margin: '2rem auto', padding: '0 1.5rem' }} data-testid="dashboard-root">
                    ...
                    <td>
                      <span
                        className={`priority-badge ${
                          s.priority === 'Cold Chain' ? 'priority-coldchain' : s.priority === 'Express' ? 'priority-express' : ''
                        }`}
                      >
                        {s.priority}
                      </span>
            ...
            {vehicles.map((v) => (
              <div key={v.id} className="glass-panel" style={{ padding: '1.5rem' }} data-testid="vehicle-card">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                  ...
                  <span className={`status-badge ${v.status === 'OnRoute' ? 'status-intransit' : v.status === 'Available' ? 'status-delivered' : 'status-delayed'}`}>
                    {v.status}
                  </span>
                </div>
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <div style={{ height: '6px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{ width: `${v.fuelOrBatteryPct}%`, height: '100%', background: v.fuelOrBatteryPct > 50 ? '#10b981' : v.fuelOrBatteryPct > 20 ? '#f59e0b' : '#ef4444' }} />
                  </div>
                </div>
            ...
            {warehouses.map((wh) => (
              <div key={wh.id} className="glass-panel" style={{ padding: '1.5rem' }} data-testid="warehouse-card">
                ...
                <div style={{ marginBottom: '1rem' }}>
                  ...
                  <div style={{ height: '6px', background: 'rgba(255,255,255,0.06)', borderRadius: '3px', overflow: 'hidden' }}>
                    <div style={{ width: `${wh.occupancyPct}%`, height: '100%', background: wh.occupancyPct > 85 ? '#ef4444' : '#06b6d4' }} />
                  </div>
                </div>
      ...
      {/* MODAL: SHIPMENT DETAIL INSPECTION */}
      <div
